...
def buscar_dolar_awesomeapi():
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            return {"preco": round(bid, 4), "fonte": "AwesomeAPI (tempo real)", "horario": horario}
    except Exception:
        pass
    # Fallback: vatcomply (outra API gratuita sem chave)
    try:
        r2 = requests.get("https://api.vatcomply.com/rates?base=USD", timeout=5)
        if r2.status_code == 200:
            brl = r2.json().get("rates", {}).get("BRL", 0)
            if brl > 0:
                return {"preco": round(brl, 4), "fonte": "VatComply (tempo real)", "horario": ""}
    except Exception:
        pass
...
def buscar_precos_scraping():
    """
    Busca preços reais diretamente dos sites CEPEA e outros via scraping leve.
    Usa headers de navegador para evitar bloqueio.
    """
    headers = {
    ...
    # ── 1. Soja — API HG Brasil (plano gratuito) ──
    # Fallback: tentativa de cotação via cepea indicadores
    try:
        # Notícias de cotação de soja via API pública do HG Brasil
        r = requests.get(
            "https://api.hgbrasil.com/finance/stock_price"
            "?key=demo&symbol=SOYB34",
            timeout=6, headers=headers
        )
        # A demo key só retorna ações; usar como teste de conectividade
    except Exception:
        pass

    # ── 2. Dólar futuro e commodities via BrapiDev (gratuita, sem chave) ──
    try:
        # USD BRL spot
        r_brl = requests.get(
            "https://brapi.dev/api/quote/USDBRL=X?token=anonymous",
            timeout=7, headers=headers
        )
        if r_brl.status_code == 200:
            d_brl = r_brl.json()
            preco_brl = d_brl.get("results", [{}])[0].get("regularMarketPrice", 0)
            if preco_brl > 0:
                resultado["dolar_brapi"] = round(preco_brl, 4)
    except Exception:
        pass

    # ── 3. Commodities via Yahoo Finance (via brapi.dev gratuito) ──
    commodities_map = {
        "ZS=F":  "soja_cbot",    # Soja CBOT (cents/bushel)
        "ZC=F":  "milho_cbot",   # Milho CBOT
        "ZW=F":  "trigo_cbot",   # Trigo CBOT
        "KC=F":  "cafe_cbot",    # Café Arábica ICE
        "GF=F":  "boi_cme",      # Feeder Cattle CME
        "CT=F":  "algodao_ice",  # Algodão ICE
    }
    for ticker, chave in commodities_map.items():
        try:
            r_c = requests.get(
                f"https://brapi.dev/api/quote/{ticker}?token=anonymous",
                timeout=6, headers=headers
            )
            if r_c.status_code == 200:
                d_c = r_c.json()
                preco_c = d_c.get("results", [{}])[0].get("regularMarketPrice", 0)
                if preco_c and preco_c > 0:
                    resultado[chave] = preco_c
        except Exception:
            continue
